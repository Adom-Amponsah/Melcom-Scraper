...
async function updateCategory(totalProducts) {
  ...
  console.log(`✓ Category updated: ELECTRICAL APPLIANCES`)
}
...
async function uploadToSupabase() {
  try {
    const baseDir = './telefonika_files/entertainment'
    ...
    // Update category with total count
    await updateCategory(totalProducts)
    
    // Upload all products
    await uploadProducts(allProducts, new Date().toISOString())
    
    console.log(`\nCompleted upload: ${totalProducts} total products uploaded to ELECTRICAL APPLIANCES`)
