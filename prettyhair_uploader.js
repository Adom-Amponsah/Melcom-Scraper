...
async function cleanPrice(priceStr) {
  try {
    ...
    // Handle "From" prices
    if (priceStr.includes('From')) {
      priceStr = priceStr.replace('From', '')
    }
...
async function uploadToSupabase() {
  try {
    const baseDir = './prettyhair_files'
    ...
    console.log(`Processing ${data.totalProducts} products`)
    ...
    console.log(`\nCompleted upload: ${data.totalProducts} products uploaded to HAIR & COSMETICS category`)
